...
for seed in seeds:
    random.seed(seed)
    np.random.seed(seed)
    a = np.loadtxt("Data/Bejing18_norm.csv", delimiter=",")

    mar_mask = get_MAR_mask_flag(org_data=a, seed=seed)
    mnar_mask = get_MNAR_mask_flag(org_data=a, seed=seed)

    np.savetxt("Data/mask/beijing18/beijing18mar_" + str(missing_p) + "_" + str(seed) + ".csv", mar_mask, fmt="%d", delimiter=",")
    np.savetxt("Data/mask/beijing18/beijing18mnar_" + str(missing_p) + "_" + str(seed) + ".csv", mnar_mask, fmt="%d", delimiter=",")
