...
for missing_p in missing_ps:
    for seed in seeds:
        ...
        random.seed(seed)
        np.random.seed(seed)
        a = np.loadtxt("Data/UrbanTraffic_norm.csv", delimiter=",")
        ...
        np.savetxt("Data/mask/urbantraffic/urbantraffic_" + str(missing_p) + "_" + str(seed) + ".csv", mask_target, fmt="%d", delimiter=",")
