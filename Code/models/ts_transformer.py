...
class TSTransformerEncoder(nn.Module):
    ...
    def get_random_mask(self, data_batch, mask_batch, mask_ratio_ssl, avg_mask_len):
        ...
        mask_temp = torch.ones_like(mask_batch)
    ...
    def get_random_block_mask(self, data_batch, mask_batch, mask_ratio_ssl, avg_mask_len):
        ...
        for sample_id in range(mask_batch.shape[0]):
            ...
            for j in range(mask.shape[1]):  
                choice_num = int(mask.shape[0] * mask_ratio_ssl / avg_mask_len) # block number
    ...
    def get_speed_mask(self, data_batch, mask_batch, mask_ratio_ssl, avg_mask_len):
        mask_ratio_ssl = mask_ratio_ssl + self.configs.missing_rate
        data_batch = data_batch.detach().to("cpu").numpy() 
        mask_batch = mask_batch.detach().to("cpu").numpy()

        mask_ssl = []
        for sample_id in range(data_batch.shape[0]):
            data = data_batch[sample_id]
            mask = mask_batch[sample_id]

            # speed = np.abs(np.diff(data, prepend=0.0001, axis=0))
       
            diff = np.diff(data, prepend=0.0001, axis=0)
            speed = np.zeros_like(data)
            for i in range(1, speed.shape[0]-1):
                speed[i] = np.abs(diff[i+1] -diff[i])
            # np.savetxt("speed.csv", speed, delimiter=",", fmt="%.6f")

            speed[np.where(speed < 0.1)] = 0.1 
            speed = speed * mask    ## set missing position's speed change to 0 (lower probability to mask)

            #for each attribute generate independent mask
            for j in range(data.shape[1]):    
                speed_j = speed[:, j] 
                speed_sum = np.sum(speed[:, j])
                
                if speed_sum == 0:
                    continue

                probability = speed_j / speed_sum

                choice_num = int(data.shape[0] * mask_ratio_ssl / avg_mask_len)
                mask_indexs = np.random.choice(speed.shape[0], choice_num, p = probability.ravel()) 
                #higher speed change ,higher mask probability
    ...
    def self_supervised_learning(self, X, mask):
        ssl_label = X * mask
        #speed mask should know the observation
        if self.configs.mask_strategy_ssl == "speed":
            mask_ssl = self.get_speed_mask(ssl_label, mask, mask_ratio_ssl=self.configs.mask_ratio_ssl, 
                                       avg_mask_len=self.configs.avg_mask_len_ssl)
        elif self.configs.mask_strategy_ssl == "tst":
            mask_ssl = self.get_tst_mask(ssl_label, mask, mask_ratio_ssl=self.configs.mask_ratio_ssl, 
                                       avg_mask_len=self.configs.avg_mask_len_ssl)
        elif self.configs.mask_strategy_ssl == "random":
            mask_ssl = self.get_random_mask(ssl_label, mask, mask_ratio_ssl=self.configs.mask_ratio_ssl, 
                                       avg_mask_len=self.configs.avg_mask_len_ssl)
        elif self.configs.mask_strategy_ssl == "randblock":
            mask_ssl = self.get_random_block_mask(ssl_label, mask, mask_ratio_ssl=self.configs.mask_ratio_ssl, 
                                       avg_mask_len=self.configs.avg_mask_len_ssl)
