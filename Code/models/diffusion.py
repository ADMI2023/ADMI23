...
class diff_ADMI(nn.Module):
    def __init__(self, configs, pretrain_model):
        ...
        if self.configs.pretrain_encoder:
            self.pretrain_timesnet = pretrain_model
        else:
            print("not pretrain")
            self.pretrain_timesnet = TSTransformerEncoder(configs)
        if self.configs.finetune_encoder == False:
            print("Not fine tuning")
            for param in self.pretrain_timesnet.parameters():
    ...
    def forward(self, total_input, side_info, cond_obs, diffusion_step):
        ...
        #condition guide
        conditions = self.pretrain_timesnet.encoder(cond_obs.permute(0,2,1)).permute(0,2,1) #[B, d, L]
